...
export const GlobalChatNotifications = () => {
  ...
  const { preferences, browserNotificationPermission } =
  ...
  const showBrowserNotification = (
    ...
    try {
      ...
      notification.onclick = () => {
        console.log(
          "🖱️ Browser notification clicked, navigating to chat:",
          chatId,
        );
    ...
    toast.info(
      ...
      </div>,
      {
        action: {
          ...
          onClick: () => {
            console.log(
              "🖱️ Toast notification clicked, navigating to chat:",
              chatId,
            );
            navigate("/crm", { state: { openChatId: chatId } });
          },
  ...
  const handleNewMessageNotification = (notification: WebSocketNewMessage) => {
    ...
    if (preferences.filterAssignedOnly) {
      if (!assigned_agent_id || assigned_agent_id !== user?.id) {
        console.log(
          "⏭️ Skipping notification - chat not assigned to current user",
        );
        return;
      }
    }
  ...
  useEffect(() => {
    console.log(
      "📡 GlobalChatNotifications: Subscribing to WebSocket messages",
    );
    ...
    return () => {
      console.log(
        "📡 GlobalChatNotifications: Unsubscribing from WebSocket messages",
      );
      unsubscribe();
